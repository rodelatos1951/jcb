 ...
 * @since  1.6
 */
class SermonModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_sermondistributor/assets/css/sermon.css'
 	];
	...
	protected array $scripts = [
		...
		'media/com_sermondistributor/js/sermon.js'
 	];
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'sermon', $prefix = 'Administrator', $config = [])
	{
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('sermon.delete', 'com_sermondistributor.sermon.' . (int) $record->id);
	}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_sermondistributor.edit.sermon.data', []);
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				if ($table->load(['alias' => $data['alias'], 'catid' => $data['catid']]) && ($table->id != $data['id'] || $data['id'] == 0))
				{
