 ...
 * @since  1.6
 */
class PreacherModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_sermondistributor/assets/css/preacher.css'
 	];
	...
	protected array $scripts = [
		...
		'media/com_sermondistributor/js/preacher.js'
 	];
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'preacher', $prefix = 'Administrator', $config = [])
	{
	...
	public function selectionTranslationVvvsermons($value,$name)
	{
		...
		if ($name === 'link_type')
		{
			...
			if (isset($link_typeArray[$value]) && UtilitiesStringHelper::check($link_typeArray[$value]))
			{
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('preacher.delete', 'com_sermondistributor.preacher.' . (int) $record->id);
	}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_sermondistributor.edit.preacher.data', []);
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				{
					$msg = Text::_('COM_SERMONDISTRIBUTOR_PREACHER_SAVE_WARNING');
				}
