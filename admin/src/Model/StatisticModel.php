...
class StatisticModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_sermondistributor/assets/css/statistic.css'
 	];
	...
	protected array $scripts = [
		...
		'media/com_sermondistributor/js/statistic.js'
 	];
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'statistic', $prefix = 'Administrator', $config = [])
	{
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('statistic.delete', 'com_sermondistributor.statistic.' . (int) $record->id);
	}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_sermondistributor.edit.statistic.data', []);
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		while (!empty($pks))
		{
			...
			// Only for strings
			if (UtilitiesStringHelper::check($this->table->filename) && !is_numeric($this->table->filename))
			{
				$this->table->filename = $this->generateUnique('filename',$this->table->filename);
			}
