 ...
 * @since  1.6
 */
class SermonField extends ListField
{
	/**
	...
	protected function getInput()
	{
		...
		if ($set_button === 'true')
		{
			...
			if ($user->authorise('sermon.create', 'com_sermondistributor') && $app->isClient('administrator')) // TODO for now only in admin area.
			{
				// build Create button
				...
				$script[] = "
					...
					function ".$button_code_name."Button(value) {
						...
						if (value > 0) {
							...
							let url = 'index.php?option=com_sermondistributor&view=sermons&task=sermon.edit&id='+value+'".$refJ."';
	...
	protected function getOptions()
	{
		...
		if ($items)
		{
			$options[] = Html::_('select.option', '', 'Select a sermon');
			foreach($items as $item)
			{
				$options[] = Html::_('select.option', $item->id, $item->sermon_name);
			}
		}
		return $options;
	}
