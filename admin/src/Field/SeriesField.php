...
class SeriesField extends ListField
{
	...
	protected function getInput()
	{
		...
		if ($set_button === 'true')
		{
			...
			if ($user->authorise('series.create', 'com_sermondistributor') && $app->isClient('administrator')) // TODO for now only in admin area.
			{
				// build Create button
				...
				$script[] = "
					...
					function ".$button_code_name."Button(value) {
						...
						if (value > 0) {
							...
							let url = 'index.php?option=com_sermondistributor&view=all_series&task=series.edit&id='+value+'".$refJ."';
	...
	protected function getOptions()
	{
		...
		if ($items)
		{
			...
			{
				$options[] = Html::_('select.option', '', Text::_('COM_SERMONDISTRIBUTOR_SELECT_A_SERIES'));
			}
			foreach($items as $item)
			{
				$options[] = Html::_('select.option', $item->id, $item->series_name);
			}
		}
