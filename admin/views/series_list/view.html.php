 ...
 */
class SermondistributorViewSeries_list extends HtmlView
{
	/**
	...
	function display($tpl = null)
	{
		...
		$this->canBatch = ($this->canDo->get('series.batch') && $this->canDo->get('core.batch'));
	...
	protected function addToolBar()
	{
		...
		ToolbarHelper::title(Text::_('COM_SERMONDISTRIBUTOR_SERIES_LIST'), 'folder-open');
		FormHelper::addFieldPath(JPATH_COMPONENT . '/models/fields');

		if ($this->canCreate)
		{
			ToolbarHelper::addNew('series.add');
		}
		...
		if (ArrayHelper::check($this->items))
		{
			if ($this->canEdit)
			{
				ToolbarHelper::editList('series.edit');
			}

			if ($this->canState)
			{
				ToolbarHelper::publishList('series_list.publish');
				ToolbarHelper::unpublishList('series_list.unpublish');
				ToolbarHelper::archiveList('series_list.archive');

				if ($this->canDo->get('core.admin'))
				{
					ToolbarHelper::checkin('series_list.checkin');
				}
			}
			...
			{
				ToolbarHelper::deleteList('', 'series_list.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
			elseif ($this->canState && $this->canDelete)
			{
				ToolbarHelper::trash('series_list.trash');
			}

			if ($this->canDo->get('core.export') && $this->canDo->get('series.export'))
			{
				ToolbarHelper::custom('series_list.exportData', 'download', '', 'COM_SERMONDISTRIBUTOR_EXPORT_DATA', true);
			}
		}

		if ($this->canDo->get('core.import') && $this->canDo->get('series.import'))
		{
			ToolbarHelper::custom('series_list.importData', 'upload', '', 'COM_SERMONDISTRIBUTOR_IMPORT_DATA', false);
		}
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(Text::_('COM_SERMONDISTRIBUTOR_SERIES_LIST'));
		Html::_('stylesheet', "administrator/components/com_sermondistributor/assets/css/series_list.css", ['version' => 'auto']);
	}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => Text::_('COM_SERMONDISTRIBUTOR_SERIES_NAME_LABEL'),
