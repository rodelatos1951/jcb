 ...
 */
class SermondistributorViewSermons extends HtmlView
{
	/**
	...
	function display($tpl = null)
	{
		...
		$this->canBatch = ($this->canDo->get('sermon.batch') && $this->canDo->get('core.batch'));
	...
	protected function addToolBar()
	{
		...
		ToolbarHelper::title(Text::_('COM_SERMONDISTRIBUTOR_SERMONS'), 'book');
		FormHelper::addFieldPath(JPATH_COMPONENT . '/models/fields');

		if ($this->canCreate)
		{
			ToolbarHelper::addNew('sermon.add');
		}
		...
		if (ArrayHelper::check($this->items))
		{
			if ($this->canEdit)
			{
				ToolbarHelper::editList('sermon.edit');
			}

			if ($this->canState)
			{
				ToolbarHelper::publishList('sermons.publish');
				ToolbarHelper::unpublishList('sermons.unpublish');
				ToolbarHelper::archiveList('sermons.archive');

				if ($this->canDo->get('core.admin'))
				{
					ToolbarHelper::checkin('sermons.checkin');
				}
			}
			...
			{
				ToolbarHelper::deleteList('', 'sermons.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
			elseif ($this->canState && $this->canDelete)
			{
				ToolbarHelper::trash('sermons.trash');
			}

			if ($this->canDo->get('core.export') && $this->canDo->get('sermon.export'))
			{
				ToolbarHelper::custom('sermons.exportData', 'download', '', 'COM_SERMONDISTRIBUTOR_EXPORT_DATA', true);
			}
		}

		if ($this->canDo->get('core.import') && $this->canDo->get('sermon.import'))
		{
			ToolbarHelper::custom('sermons.importData', 'upload', '', 'COM_SERMONDISTRIBUTOR_IMPORT_DATA', false);
		}
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			JHtmlBatch_::addListSelection(
				Text::_('COM_SERMONDISTRIBUTOR_KEEP_ORIGINAL_CATEGORY'),
				'batch[category]',
				Html::_('select.options', Html::_('category.options', 'com_sermondistributor.sermon'), 'value', 'text')
			);
		}
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_SERMONDISTRIBUTOR_SERMON_PREACHER_LABEL').' -',
				'batch[preacher]',
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_SERMONDISTRIBUTOR_SERMON_SERIES_LABEL').' -',
				'batch[series]',
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->link_typeOptions = FormHelper::loadFieldType('sermonsfilterlinktype')->options;
			// We do some sanitation for Link Type filter
			if (ArrayHelper::check($this->link_typeOptions) &&
				isset($this->link_typeOptions[0]->value) &&
				!StringHelper::check($this->link_typeOptions[0]->value))
			{
			...
			JHtmlBatch_::addListSelection(
				...
				'batch[link_type]',
				Html::_('select.options', $this->link_typeOptions, 'value', 'text')
			);
		}
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->sourceOptions = FormHelper::loadFieldType('sermonsfiltersource')->options;
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_SERMONDISTRIBUTOR_SERMON_SOURCE_LABEL').' -',
				'batch[source]',
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(Text::_('COM_SERMONDISTRIBUTOR_SERMONS'));
		Html::_('stylesheet', "administrator/components/com_sermondistributor/assets/css/sermons.css", ['version' => 'auto']);
	}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => Text::_('COM_SERMONDISTRIBUTOR_SERMON_NAME_LABEL'),
			'g.name' => Text::_('COM_SERMONDISTRIBUTOR_SERMON_PREACHER_LABEL'),
			'h.name' => Text::_('COM_SERMONDISTRIBUTOR_SERMON_SERIES_LABEL'),
			'a.short_description' => Text::_('COM_SERMONDISTRIBUTOR_SERMON_SHORT_DESCRIPTION_LABEL'),
			'category_title' => Text::_('COM_SERMONDISTRIBUTOR_SERMON_SERMONS_CATEGORIES'),
			'a.link_type' => Text::_('COM_SERMONDISTRIBUTOR_SERMON_LINK_TYPE_LABEL'),
			'a.source' => Text::_('COM_SERMONDISTRIBUTOR_SERMON_SOURCE_LABEL'),
