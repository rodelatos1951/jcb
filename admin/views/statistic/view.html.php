...
class SermondistributorViewStatistic extends HtmlView
{
	...
	protected function addToolBar()
	{
		...
		ToolbarHelper::title( Text::_($isNew ? 'COM_SERMONDISTRIBUTOR_STATISTIC_NEW' : 'COM_SERMONDISTRIBUTOR_STATISTIC_EDIT'), 'pencil-2 article-add');
		...
		if (StringHelper::check($this->referral))
		{
			if ($this->canDo->get('statistic.create') && $isNew)
			{
				...
				ToolbarHelper::save('statistic.save', 'JTOOLBAR_SAVE');
			}
			...
			if ($isNew)
			{
				...
				ToolbarHelper::cancel('statistic.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				// We can close it.
				ToolbarHelper::cancel('statistic.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			if ($isNew)
			{
				...
				{
					ToolbarHelper::apply('statistic.apply', 'JTOOLBAR_APPLY');
					ToolbarHelper::save('statistic.save', 'JTOOLBAR_SAVE');
					ToolbarHelper::custom('statistic.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
				};
				ToolbarHelper::cancel('statistic.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				...
				{
					ToolbarHelper::versions('com_sermondistributor.statistic', $this->item->id);
				}
				if ($this->canDo->get('statistic.create'))
				{
					ToolbarHelper::custom('statistic.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
				}
				ToolbarHelper::cancel('statistic.cancel', 'JTOOLBAR_CLOSE');
			}
		}
	...
	protected function setDocument()
	{
		...
		Html::_('stylesheet', "administrator/components/com_sermondistributor/assets/css/statistic.css", ['version' => 'auto']);
		Html::_('script', $this->script, ['version' => 'auto']);
		Html::_('script', "administrator/components/com_sermondistributor/views/statistic/submitbutton.js", ['version' => 'auto']);
